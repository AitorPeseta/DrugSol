...
def _reorder_X_to_feature_list(df: pd.DataFrame, features: List[str]) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"[FATAL] Faltan columnas requeridas por el modelo GBM: {missing[:12]}{'...' if len(missing)>12 else ''}")
    ...
    if isinstance(stack_obj, dict):
        ...
        if coef is not None:
            ...
            if feat_names is not None:
                ...
                if missing:
                    raise ValueError(f"[FATAL] stack_pkl pide columnas inexistentes: {missing}")
                X_meta = level0_out[feat_names].to_numpy(dtype=float)
                if len(coef) != len(feat_names):
                    raise ValueError(f"[FATAL] len(coef)={len(coef)} != len(feature_names)={len(feat_names)}")
            else:
                if len(coef) != len(pred_cols):
                    raise ValueError(f"[FATAL] len(coef)={len(coef)} != len(pred_cols)={len(pred_cols)} ({pred_cols})")
                X_meta = X_meta_default
            return X_meta.dot(coef) + intercept

        weights = d.get("weights", None)
        if isinstance(weights, dict):
            alias = {"xgb":"y_xgb","lgbm":"y_lgbm","gnn":"y_chemprop","chemprop":"y_chemprop","tpsa":"y_tpsa"}
            w_by_col = {}
        ...
        if isinstance(wf, dict):
            alias = {"xgb":"y_xgb","lgbm":"y_lgbm","gnn":"y_chemprop","chemprop":"y_chemprop","tpsa":"y_tpsa",
                     "y_xgb":"y_xgb","y_lgbm":"y_lgbm","y_chemprop":"y_chemprop","y_tpsa":"y_tpsa"}
...
def _maybe_make_gbm_preds_from_pkls(models_dir: Path,
    ...
    base_df = test_tab.copy()
    drop_cols = [id_col] + ([target] if target and target in base_df.columns else []) + \
                ["smiles", "smiles_neutral", "SMILES", "Smiles"]
...
def _maybe_add_tpsa_pred(out_df: pd.DataFrame,
    ...
    """
    Si tpsa_json se proporciona y existen columnas requeridas, añade y_tpsa.
    Formato JSON esperado: {"a": ..., "b": ..., "c": ... (opcional)}
    ...
    try:
        ...
        print(f"[WARN] No se pudo leer coeficientes de {tpsa_json}: {e}")
        return out_df
    ...
    if phenol_col and phenol_col in test_tab.columns and c != 0.0:
        ...
        # si es booleano, mapear a {0,1}
        if ph.dropna().isin([0,1]).all() is False:
            # deja como conteo si no es 0/1
            pass
        extra = c * ph.values
    else:
        extra = 0.0
...
def main():
    ...
    # --- NUEVO: modelo TPSA (+fenol opcional) ---
    ap.add_argument("--tpsa-json", default=None, type=str,
                    help="JSON con coeficientes del modelo TPSA: {'a':..., 'b':..., 'c': opcional}.")
    ap.add_argument("--tpsa-col", default="TPSA", type=str,
                    help="Nombre de columna TPSA en test tabular.")
    ap.add_argument("--phenol-col", default="phenol_count", type=str,
                    help="Nombre de columna del descriptor de fenoles (booleano o conteo).")
    ...
    _must_have(test_smi, args.smiles_col if args.chemprop_smiles_col is None else _pick_smiles_col(test_smi, args.chemprop_smiles_col, args.smiles_col), "test_smi (smiles)")
    ...
    base_tab = _maybe_make_gbm_preds_from_pkls(Path(args.models_dir), test_tab, args.id_col, args.target, Path(args.save_dir))
    _must_be_unique(base_tab, args.id_col, "base_tab")
    print(f"[DEBUG] base_tab rows={len(base_tab)} (ids únicos)")

    # -------------------- TPSA (+fenol) base (opcional) -----------------------
    ...
    # -------------------- chemprop predict / o cargar preds -------------------
    if args.chemprop_preds:
        ...
        if args.id_col in cp_raw.columns:
            ...
            test_key = _smiles_cumkey(test_smi[[args.id_col, args.smiles_col]].copy(), args.smiles_col, "_cum") \
                           .rename(columns={args.smiles_col: "smiles"})
            cp_key = _smiles_cumkey(cp_raw[["smiles", pred_col]].rename(columns={"smiles":"smiles"}), "smiles", "_cum")
            tmp = test_key.merge(cp_key.rename(columns={pred_col:"y_chemprop"}), on=["smiles","_cum"], how="left")
            cp_preds = tmp[[args.id_col, "y_chemprop"]].copy()
        else:
            raise ValueError("Formato de --chemprop-preds no reconocido. Esperado (row_uid,y_chemprop) o (smiles,y_chemprop).")
        ...
        need_cols = [chem_col, "temp_C"]
        missing = [c for c in need_cols if c not in test_smi.columns]
        if missing:
            raise ValueError(f"Faltan columnas en test_smi: {missing}")

        tmp_rows = test_smi[[chem_col, "temp_C"]].copy()
        ...
        tmp_keyed[["smiles","temp_C"]].to_csv(tmp_test_csv, index=False)
    ...
    # Diagnóstico ids fuera de test_tab
    ids_tab = set(_norm_id(test_tab[args.id_col]))
    ids_cp = set(_norm_id(cp_preds[args.id_col]))
    only_tab = ids_tab - ids_cp
    only_cp = ids_cp - ids_tab
    print(f"[DEBUG] IDs sólo en TAB: {len(only_tab)} | IDs sólo en CP: {len(only_cp)}")
    _dump_ids(Path(args.save_dir) / "debug_ids" / "ids_only_in_tab.csv", pd.Series(list(only_tab)), "ids_only_in_tab")
    _dump_ids(Path(args.save_dir) / "debug_ids" / "ids_only_in_cp.csv", pd.Series(list(only_cp)), "ids_only_in_cp")
    ...
    if has_y:
        ...
        base_mask  = merged_eval[[c for c in ["y_xgb","y_lgbm","y_tpsa"] if c in merged_eval.columns]].notna().any(axis=1) \
                     if set(["y_xgb","y_lgbm","y_tpsa"]).intersection(merged_eval.columns) else pd.Series(False, index=merged_eval.index)
        ...
        for cohort_name, m in [("base_all", base_mask), ("chemprop", cp_mask), ("inner", inner_mask), ("union", union_mask)]:
            for col in [c for c in ["y_xgb", "y_lgbm", "y_chemprop", "y_tpsa"] if c in merged_eval.columns]:
                res = _eval_mask(cohort_name, m, col)
                if res is not None:
